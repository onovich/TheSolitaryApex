...
Route and level pacing live in `src/data/levelConfig.js`. Each level has authoring metadata that separates authored controls from randomized controls, content targets for generated mechanic counts, plus pressure rules for event spacing. Each level also has a stable `seed` so route changes are reproducible while tuning. The HUD level switcher restarts into any configured official template. Use `npm run validate:levels` to check level definitions, content targets, pressure rules, generated-route coverage, seed repeatability, and a short per-template pressure summary before committing route template changes.
