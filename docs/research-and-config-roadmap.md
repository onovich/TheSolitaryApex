...
- Developer workflow:
  ...
  - Current status: per-frame engine orchestration now lives in `src/logic/engine/frameUpdateSystem.js`, dyno flight and auto-attach frame dispatch lives in `src/logic/engine/frameAirborneUpdateSystem.js`, shared tail ticks live in `src/logic/engine/framePostUpdateSystem.js`, and `gameEngine.js` preserves the public `updateFrame` export.
