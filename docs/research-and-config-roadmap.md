...
- `src/logic/engine/initialStateSystem.js`
  - Movement, debug, feedback, item, route, spatial-scan, fall, and recovery state factories.
...
- `src/logic/engine/gameStateFactory.js`
  - Full run initialization, loadout inventory setup, route analysis snapshot creation, and initial aggregate game-state assembly.
...
- Level config contract:
  ...
  - Current status: seeded wall generation wrapper now lives in `src/logic/engine/routeGeneration.js`, final wall-blueprint assembly lives in `src/logic/engine/routeBlueprintGeneration.js`, route path scaffolding split into `src/logic/engine/routePathGeneration.js`, route segment lookup split into `src/logic/engine/routeSegmentGeneration.js`, randomized route content split into `src/logic/engine/routeContentGeneration.js`, authored route content split into `src/logic/engine/routeAuthoredContentGeneration.js`, and seeded-random/hold primitives split into `src/logic/engine/routeGenerationPrimitives.js`.
