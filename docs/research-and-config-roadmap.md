...
- `src/logic/engine/routeBlueprintPathAssembly.js`
- `src/logic/engine/routeBlueprintStanceAssembly.js`
  - Route blueprint path-content assembly from spawn holds, Golden Path stances, randomized noise holds, and route segments, with single-stance Golden/noise hold appending split out of the top-level path assembly and `routeBlueprintGeneration.js` preserving final authored-content and metadata assembly.
...
- `src/logic/engine/fallBeginSystem.js`
  - Failure-to-fall entry orchestration and transient climbing-state cleanup.
- `src/logic/engine/fallBeginStateSystem.js`
  - Checkpoint fall stamina penalties, checkpoint rope-fall state creation, and death-fall state creation.
...
- `src/logic/engine/failureSystem.js`
  - Failure-to-fall routing, fall/dyno reset, invincible-failure facade exports, and stable game-over facade export.
- `src/logic/engine/gameOverSystem.js`
  - Game-over finalization, transient input cleanup, fall/dyno/feedback/recovery reset, and final run summary creation.
...
- `src/logic/engine/frameAirborneUpdateSystem.js`
  - Per-frame dyno flight and auto-attach phase dispatch, with shared airborne tail ticks.
- `src/logic/engine/frameClimbingUpdateSystem.js`
  - Per-frame stable-climbing phase dispatch for anchor sync, stamina/failure checks, body motion, and camera updates.
...
- `src/logic/engine/gameStateRuntimeFields.js`
  - Runtime default field assembly for player, pointer, stamina, inventory, subsystem states, spatial scan, route state, and UI flags.
...
- `src/logic/engine/gameEngineActionFacade.js`
- `src/logic/engine/gameEngineFailureActions.js`
- `src/logic/engine/gameEngineDragActions.js`
- `src/logic/engine/gameEngineBodyDynoActions.js`
- `src/logic/engine/gameEngineItemFrameActions.js`
  - Public engine action wrapper assembly split by failure, drag, body/dyno, and item/frame domains, keeping `gameEngine.js` as a small stable export facade.
...
- Developer workflow:
  ...
  - Current status: failure-to-fall entry orchestration now lives in `src/logic/engine/fallBeginSystem.js`, fall-begin state factories live in `src/logic/engine/fallBeginStateSystem.js`, checkpoint pose restoration lives in `src/logic/engine/checkpointPoseRecoverySystem.js`, `src/logic/engine/fallEntrySystem.js` preserves stable facade exports, fall-mode dispatch lives in `src/logic/engine/fallRecoverySystem.js`, death-fall updates live in `src/logic/engine/deathFallSystem.js`, rope catch updates live in `src/logic/engine/ropeFallCatchSystem.js`, and hanging rope recovery lives in `src/logic/engine/hangingRecoverySystem.js`.
  ...
  - Current status: failure-to-fall routing and fall/dyno reset now live in `src/logic/engine/failureSystem.js`, game-over finalization lives in `src/logic/engine/gameOverSystem.js`, while invincible debug toggling lives in `src/logic/engine/invincibleFailureSystem.js`, invincible recovery orchestration lives in `src/logic/engine/invincibleRecoverySystem.js`, reachable attachment orchestration lives in `src/logic/engine/invincibleAttachmentRecoverySystem.js`, fallback hold search lives in `src/logic/engine/invincibleAttachmentSearchSystem.js`, and recovered-limb attachment application lives in `src/logic/engine/invincibleAttachmentApplySystem.js`.
  - Current status: per-frame engine orchestration now lives in `src/logic/engine/frameUpdateSystem.js`, dyno flight and auto-attach frame dispatch lives in `src/logic/engine/frameAirborneUpdateSystem.js`, stable-climbing frame dispatch lives in `src/logic/engine/frameClimbingUpdateSystem.js`, shared tail ticks live in `src/logic/engine/framePostUpdateSystem.js`, and `gameEngine.js` preserves the public `updateFrame` export.
  - Current status: cross-system runtime adapter wiring now lives in `src/logic/engine/gameEngineRuntime.js`, public engine action wrappers are split into `src/logic/engine/gameEngineActionFacade.js` plus domain action modules, frame/fall composition lives in `src/logic/engine/gameRuntimeAdapters.js` and `src/logic/engine/gameRuntimeFallAdapters.js`, fall dependency bundles are split into `src/logic/engine/gameRuntimeEncounterAdapter.js`, `src/logic/engine/gameRuntimeFallRecoveryAdapter.js`, and `src/logic/engine/gameRuntimeFailureAdapter.js`, and interaction dependency bundles are split into `src/logic/engine/gameRuntimeInteractionAdapters.js`, `src/logic/engine/gameRuntimeHoldInteractionAdapter.js`, `src/logic/engine/gameRuntimeMovementAdapters.js`, and `src/logic/engine/gameRuntimeItemAdapter.js`, keeping `gameEngine.js` focused on stable public facade exports.
  ...
  - Current status: initial run content generation, debug filtering, and route analysis snapshot creation now live in `src/logic/engine/gameInitialRunContent.js`, runtime default fields live in `src/logic/engine/gameStateRuntimeFields.js`, aggregate state assembly lives in `src/logic/engine/gameStateFactory.js`, and debug-run option resolution and event filtering live in `src/logic/engine/gameRunDebugSystem.js`.
